...
def calculate(list):
    if len(list) < 9:
        ...
        max_axis1 = three_x_matrx.max(axis=1)
        max_axis2 = three_x_matrx.max(axis=0)

        min_flat = flat_matrix.min()
        min_axis1 = three_x_matrx.min(axis=1)
        min_axis2 = three_x_matrx.min(axis=0)

        sum_flat = flat_matrix.sum()
        sum_axis1 = three_x_matrx.sum(axis=1)
        sum_axis2 = three_x_matrx.sum(axis=0)

        calculations = {
            'mean': [mean_axis2, mean_axis1, mean_flat],
            'variance': [var_axis2, var_axis1, var_flat],
            'standard deviation': [std_axis2, std_axis1, std_flat],
            'max': [[x for x in max_axis2], [x for x in max_axis1], max_flat],
            'min': [[x for x in min_axis2], [x for x in min_axis1], min_flat],
            'sum': [[x for x in sum_axis2], [x for x in sum_axis1], sum_flat]
            }
